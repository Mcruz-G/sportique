...
def build_blp_dataset(game_ids, teams_data):
...
def train_blp_model(blp_dataset, target, full_training=False):
    ...
    model.fit(X, y)
    return model, categorical_encoder

if __name__ == "__main__":

    model_name = "BLPModel"
    model_dir = os.environ["SP_MODELS_PATH"] + f"{model_name}/"
    model_path = model_dir + f"{model_name}.sav"
    categorical_encoder_path = model_dir + "categorical_encoder.sav"

    game_ids, teams_data= load_game_ids().drop_duplicates(), load_teams_data()
    blp_dataset = build_blp_dataset(game_ids, teams_data)
    model, categorical_encoder = train_blp_model(blp_dataset, "NAIVE_PRED_prob", full_training=True)


    save_model(model, model_path)
    save_model(categorical_encoder, categorical_encoder_path)
