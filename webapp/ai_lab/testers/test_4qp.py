...
from utils import get_4qp_test_samples, load_model, model_paths
import matplotlib.pyplot as plt


def test_4qp(model_name):
    h2o.init()

    game_ids = load_game_ids()
    game_ids = game_ids.iloc[:int(len(game_ids)*0.8)]
    test_data = get_4qp_test_samples(game_ids)
    print(test_data.columns)
    model_path = model_paths["4QPModel"]
